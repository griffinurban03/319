...
const averagePerProduct = function(productName) {
    let productAverage = 0;
    let productTotal = 0;
    
    for (let product in products) {
        if (products[product][1] == productName) {
            productAverage += products[product][6];
            productTotal += 1;
        }
    }
    return (productAverage / productTotal).toFixed(2);
}
